...
class ProjectionTracker(BaseTracker):
    ...
    def track_frames(self, raw_masks: List[np.ndarray], frames: List[np.ndarray]) -> List[np.ndarray]:
        ...
        plt.imshow(projection_mask)
        plt.axis("off")
        plt.show()


        lines = cv2.HoughLinesP(projection_mask.astype(np.uint8),
                                rho=1,
                                theta=np.pi / 180,
                                threshold=150,
                                minLineLength=30,
        ...
        instance_masks = np.zeros_like(projection_mask, dtype=np.uint8)

        for i, line in enumerate(lines.reshape(-1, 4)):
            x1, y1, x2, y2 = line
            cv2.line(instance_masks, (x1, y1), (x2, y2), i, thickness=5)

        plt.imshow(instance_masks)
        plt.title("Instances Mask")
        plt.show()
