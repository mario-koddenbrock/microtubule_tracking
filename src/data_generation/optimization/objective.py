...
def objective(
    ...
    try:
        ...
        # Catch validation errors from create_synthetic_config_from_trial or other config issues
        logger.error(f"Trial {trial.number}: Configuration or data generation failed due to validation error: {e}",
                     exc_info=False)
        ...
        logger.error(f"Trial {trial.number}: An unexpected error occurred during objective evaluation: {e}",
                     exc_info=True)
        score = -float('inf')  # Assign a very bad score
